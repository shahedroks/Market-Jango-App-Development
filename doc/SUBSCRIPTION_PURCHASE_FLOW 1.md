...
2. **Store `data.tx_ref`** (you will need it for the fallback below).
3. On success, open `data.payment_url` in a **browser** or **in-app webview**.
4. User completes payment on Flutterwave. Flutterwave may redirect to our backend; the backend then activates the subscription. **If the redirect does not reach the server (e.g. wrong port), the database will not update.**
5. **When the user returns to the app** (from browser/webview or deep link), call **`POST /api/subscription/confirm-payment`** with body `{ "tx_ref": "<the tx_ref you stored>" }`. The backend will verify the payment with Flutterwave and activate the subscription. This way the database is updated even when the redirect callback never runs.
6. Then call `GET /api/subscription/current` to show the new subscription.
...
### API 5: Confirm payment (fallback – use when redirect doesn’t update the database)

If after the user pays on Flutterwave the subscription still doesn’t appear (e.g. redirect didn’t hit your API), the app can **confirm the payment** with this API. The backend will verify the transaction with Flutterwave and then activate the subscription.

**When:** Right after the user returns to the app from the Flutterwave payment page (or when you detect they finished payment). Use the same `tx_ref` you got from the initiate-payment response.

**Request:**
- **Method:** `POST`
- **URL:** `/api/subscription/confirm-payment`
- **Headers:** `token: Bearer {jwt_token}`, `Content-Type: application/json`
- **Body (JSON):**
```json
{
  "tx_ref": "sub_7_3_20260216053506_9SEBPQ"
}
```
...
**Response – already confirmed (200):** Same structure with message "Payment already confirmed and subscription active."

**Response – not found (404):** "Pending payment not found or already processed."

**Response – verification failed (400):** "Payment could not be verified." (e.g. user didn’t complete payment or Flutterwave returned not successful).
...
| **Confirm payment (fallback)** | **POST** | **`/api/subscription/confirm-payment`** | **When user returns from Flutterwave (if DB not updated)** |
